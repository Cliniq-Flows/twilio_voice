...
// swift_copy.swift  — updated with conference, display-name, custom-params, and ringback tone support
...
//        self.incomingPushCompletionCallback = completion
    ...
    public func callDidConnect(call: Call) {
        ...
        sendPhoneCallEvents(
        description: "DIAG|post-connect route=\(s.currentRoute.outputs.map{$0.portType.rawValue}) mode=\(s.mode.rawValue)",
        isError: false
        )
    ...
    public func provider(_ provider: CXProvider, perform action: CXStartCallAction) {
        ...
        // Begin ringback for outbound call
        wantsRingback = true
        if callkitAudioActive && ringtonePlayer == nil { playRingbackTone() }
    ...
    // ===== Added features from enhanced plugin =====
    // Ringback helpers + display name + custom params persistence + conference connect
    ...
    private func playRingbackTone() {
        ...
        // Ensure Twilio’s audio session config
        audioDevice.block()
        let s = AVAudioSession.sharedInstance()
        sendPhoneCallEvents(
            description: "DIAG|ringback pre|cat=\(s.category.rawValue) mode=\(s.mode.rawValue) opt=\(s.categoryOptions) route=\(s.currentRoute.outputs.map{$0.portType.rawValue})",
            isError: false
        )
    ...
    func updateCurrentCallDisplayName(to newName: String) {
        guard let activeCall = self.call else { return }
    ...
    func connectToConference(
        ...
        // Pass conf name via params and start via CallKit (per Twilio flow)
